<<<<<<< 312ff70bd890ceedd204c9e420ae7e91a8909661
#include <Servo.h>
...
void load_config(){
  ...
  Serial1.println("dev config");
    ...
    Serial1.println(dev_config.smoke_active);
  #endif
}
...
void setup(){
  ...
  if(!digitalRead(MODE_SW)){
    ...
    //Si se rompe el umbral se sale de la función setup para continuar con loop 
    if(smoke_adc > dev_config.smoke_thresh && dev_config.smoke_active){
      #ifdef DEBUG
        Serial1.println("Alarma por humo");
      #endif
      timer_reset();
...
>>>>>>> Actualiza archivos de manufactura
